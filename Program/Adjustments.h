...
/* 
 *  Modified version of Spf::Adjustments:
 */
...
namespace Spf {
	...
	public:
		Adjustments(const EngineParamsType& engineParams,size_t maxIter=100,FieldType tolerance=1.0e-3) :
			engineParams_(engineParams),nsites_(engineParams_.lx*engineParams_.ly),maxIter_(maxIter),tolerance_(tolerance)
		{
		}
		...
		FieldType adjChemPot(const std::vector<FieldType>& eigs) const
		{
			...
			FieldType n0=engineParams_.density*nsites_;
			FieldType mu=eigs[int(n0)];	//--------modified--------
		...
		FieldType simpleAdjChemPot(const std::vector<FieldType>& eigs) const
		{
			FieldType n0=engineParams_.density*nsites_;
			int i_fs=int(n0)-1;
