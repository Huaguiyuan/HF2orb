...
namespace HF {
	...
	class MFParams{ //! Mean-Field Parameters
		...
		FieldType calcConst() const
		{
			FieldType sum=0;
			for (int i=0;i<volume_;i++) {
				sum += + uu*(norm(spx[i])+norm(spy[i])) + (up - jj/2)*(norm(tpu[i])+norm(tpd[i])+norm(opx[i])+norm(opy[i])) - 2*jj*( 2*real(tpu[i]*conj(tpd[i]) + tpd[i]*conj(tpu[i])) + norm(tpu[i])+norm(tpd[i])-norm(opx[i])-norm(opy[i])) -jj*(real(tpu[i]*tpd[i] + conj(tpd[i])*conj(tpu[i]))); //-uu*(nxu[i]*nxd[i]+nyu[i]*nyd[i]) -(up - jj/2)*(nxu[i]+nxd[i])*(nyu[i]+nyd[i]) +2*jj*(2*real(spx[i]*conj(spy[i]) + spy[i]*conj(spx[i])) + nxu[i]*nyu[i]+nxd[i]*nyd[i]-nxu[i]*nyd[i]-nxd[i]*nyu[i]) + jj*(real(opx[i]*conj(opy[i]) + opy[i]*conj(opx[i])));// 
			}
			return sum;
		}
